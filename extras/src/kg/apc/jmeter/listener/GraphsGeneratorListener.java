 ...
 * @since 1.1.3
 */
public class GraphsGeneratorListener extends AbstractListenerElement implements
		TestStateListener, TestBean, TestElement, Visualizer {

	private static final Logger log = LoggingManager.getLoggerForClass();

	public enum ExportMode {
		PNG((byte) 0), CSV((byte) 1);
	...
	public void testEnded(String host) {
		for (int i = 0; i < pluginTypes.length; i++) {
			...
			if (!StringUtils.isEmpty(successFilter)) {
				worker.setSuccessFilter("True". //$NON-NLS-1$
			...
			if (!StringUtils.isEmpty(outputBaseFolder)) {
				fileName = outputBaseFolder + File.separatorChar + filePrefix
						+ pluginTypes[i];
			} else {
			...
			if (status == 0) {
				log.info("Successful generation of file " + fileName
						+ " by plugin:" + pluginTypes[i]);
			} else {
				log.error("Error generating file " + fileName + " by plugin:"
						+ pluginTypes[i]);
			}
		}
	}
	...
	/**
	 * Override the setProperty method in order to convert the original String
	 * calcMode property. This used the locale-dependent display value, so
	 * caused problems when the language was changed. Note that the calcMode
	 * StringProperty is replaced with an IntegerProperty so the conversion only
	...
	public void setProperty(JMeterProperty property) {
		if (property instanceof StringProperty) {
			...
			if (pn.equals("exportMode")) { //$NON-NLS-1$
				...
				try {
					final BeanInfo beanInfo = Introspector.getBeanInfo(this
							.getClass());
					final ResourceBundle rb = (ResourceBundle) beanInfo
					...
					for (Enum<ExportMode> e : ExportMode.values()) {
						...
						if (objectValue.equals(rb.getObject(propName))) {
							...
							if (log.isDebugEnabled()) {
								log.debug("Converted " + pn + "=" + objectValue
										+ " to mode=" + tmpMode
	...
	/**
	 * @param preventOutliers
	 *            the preventOutliers to set
	 */
	...
	/**
	 * @param outputBaseFolder
	 *            the outputBaseFolder to set
	 */
