...
public class GraphsGeneratorListenerBeanInfo extends BeanInfoSupport {
	...
	static final String[] FILTER_TAGS = new String[] { "", "True", "False" };
	...
	public GraphsGeneratorListenerBeanInfo() {
		...
		try {
			createPropertyGroup("output_config", //$NON-NLS-1$
					new String[] { OUTPUT_BASE_FOLDER, RESULTS_FILE_NAME,
							EXPORT_MODE, FILE_PREFIX });
			...
			p.setValue(DEFAULT,
					GraphsGeneratorListener.ExportMode.PNG.ordinal());
			...
			createPropertyGroup("graphs_config", //$NON-NLS-1$
					new String[] { GRAPH_WIDTH, GRAPH_HEIGHT, PAINT_MARKERS,
							PAINT_ZEROING, PAINT_GRADIENT, PREVENT_OUTLIERS,
							RELATIVE_TIMES, AUTO_SCALE_ROWS, LIMIT_ROWS,
							FORCE_Y, GRANULATION, LINE_WEIGHT });
			...
			p.setValue(TAGS, new String[] { "True", "False" }); // $NON-NLS-1$
																// $NON-NLS-2$
