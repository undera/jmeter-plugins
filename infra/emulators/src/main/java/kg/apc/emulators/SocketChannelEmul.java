...
public class SocketChannelEmul extends SocketChannel {
    ...
    public int read(ByteBuffer dst) throws IOException {
        ...
        int cnt = bytesToRead.remaining() > dst.capacity() ? dst.capacity() : bytesToRead.remaining();
        byte[] byteArray = new byte[cnt];
        bytesToRead.get(byteArray);
        dst.put(byteArray);
        return cnt;
    }
...
//        log.debug("Set bytes to read: " + getString(wrap));
    ...
    private String getString(ByteBuffer src) {
        ...
        return FileChannelEmul.byteBufferToString(src);
    }
