...
<span class=''>[<i class='fa fa-download'></i> Download](/?search=jpgc-cmd)</span>
...
Note you can use [GraphsGeneratorListener](/wiki/GraphsGeneratorListener/) if you want to bulk generate graphs after a test or for existing results.

## Installation

The tool is installed normally with [PluginsManager](/wiki/PluginsManager/), producing launcher bat/sh files in JMeter's `bin` directory. Use `JMeterPluginsCMD.bat` on Windows and `JMeterPluginsCMD.sh` on MacOS and Linux.
...
like [GUI settings](/wiki/SettingsPanel/) do with optional parameters:
...
Java system properties can be defined via the environment variable `JVM_ARGS`, e.g. `JVM_ARGS=-Duser.timezone=GMT JMeterPluginsCMD ...`.
...
Most of class names are self-explanatory:
  ...
  - HitsPerSecond
  - LatenciesOverTime
  - PerfMon = [PerfMon](/wiki/PerfMon/) Metrics Collector
  - DbMon = [DbMon](/wiki/DbMon/) Metrics Collector, DataBase, get performance counters via sql
  - JMXMon = [JMXMon](/wiki/JMXMon/) Metrics Collector, Java Management Extensions counters
  ...
  - MergeResults = [MergeResults](/wiki/MergeResults/) Command Line Merge Tool to simplify the comparison of two or more load tests, need properties file (like merge-results.properties)
