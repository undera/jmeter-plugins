...
ServerAgent does not required to be installed into JMeter directory. You should install and launch it on the target server instead. See [server agent](https://github.com/undera/perfmon-agent/blob/master/README.md) wiki page for details.
...
JMeter-Plugins versionы  before 1.1.1 were shipped via three distribution ZIPs:
  1. JMeterPlugins-x.x.x.zip - contains plugins, must be unpacked into *lib/ext* directory inside JMeter
  1. JMeterPlugins-libs-x.x.x.zip - contains additional JARs used by JSON/HBase/Hadoop/etc plugins, must be upnacked into *lib* directory inside JMeter
  1. ServerAgent-x.x.x.zip - contains [server resource monitoring agent](https://github.com/undera/perfmon-agent/blob/master/README.md) to use with [PerfMon Metrics Collector](/wiki/PerfMon/) plugin, standalone utility
...
If you plan to use the Servers Monitoring Listener, you must also install the server agent on all servers as described [here](/wiki/PerfMon#Installation).