...
If you want to use [JMeter Plugins Manager](/wiki/PluginsManager/) in an automated fashion, you will need to make some preparations first:
  ...
  1. make sure `PluginsManagerCMD.sh` or `PluginsManagerCMD.bat` is present in `jmeter/bin` directory. If not, run `java -cp jmeter/lib/ext/jmeter-plugins-manager-x.x.jar org.jmeterplugins.repository.PluginManagerCMDInstaller` to have the files created
