...
public class PluginsCMDWorker {
	...
	private void checkParams() {
		if (pluginType == null) {
			throw new IllegalArgumentException(
					"Missing plugin type specification");
		}

		if (exportMode == 0) {
			throw new IllegalArgumentException(
					"Missing any export specification");
		}

		if (inputFile == null) {
			throw new IllegalArgumentException(
					"Missing input JTL file specification");
		}

		if (!(new File(inputFile).exists())) {
			throw new IllegalArgumentException(
					"Cannot find specified JTL file: " + inputFile);
		}
	...
	public int doJob() {
		...
		if ((exportMode & EXPORT_PNG) == EXPORT_PNG) {
			...
			try {
				pluginInstance.getGraphPanelChart().saveGraphToPNG(pngFile,
	...
	private void setOptions(AbstractGraphPanelVisualizer gui) {
		...
		if (preventOutliers >= 0) {
			graph.getChartSettings().setPreventXAxisOverScaling(
					preventOutliers > 0);
		}
		if (lowCounts >= 0) {
		...
		if (markers >= 0) {
			graph.getChartSettings().setChartMarkers(
					markers > 0 ? ChartSettings.CHART_MARKERS_YES
	...
	private void forceDir(File resultFile) {
		...
		if (parent != null) {
			if (!parent.mkdirs() && !parent.exists()) {
				throw new RuntimeException("Failed to create directory for "
	...
	public void processUnknownOption(String nextArg, ListIterator args) {
		if (pluginType != null
				&& pluginType instanceof CMDLineArgumentsProcessor) {
			log.debug("Trying to process unknown option using CMDLineArgumentsProcessor: "
					+ nextArg);
